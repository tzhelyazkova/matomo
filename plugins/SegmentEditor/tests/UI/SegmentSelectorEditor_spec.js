...
describe("SegmentSelectorEditorTest", function () {
    ...
    async function selectFieldValue(page, fieldName, textToSelect)
    {
    ...
    async function selectDimension(page, prefixSelector, category, name)
    {
        await page.click(prefixSelector + ' .metricListBlock .select-wrapper');
        await page.click(prefixSelector + ' .metricListBlock .expandableList h4:contains(' + category + ')');
        await page.click(prefixSelector + ' .metricListBlock .expandableList .secondLevel li:contains(' + name + ')');
    }
    ...
    it("should change segment when another available segment clicked in segment editor's available segments dropdown", async function() {
        await page.click('.ui-menu-item a:contains(Add new segment)');
    ...
    it("should update segment expression when selecting different segment", async function() {
        await selectDimension(page, '.segmentRow0', 'Actions', 'Action URL');
    ...
    it("should add an OR condition when a segment dimension is selected in the OR placeholder section", async function() {
        await selectDimension(page, '.segmentRow0 .segment-row:last', 'Actions', 'Clicked URL');
    ...
    it("should add an AND condition when a segment dimension is dragged to the AND placeholder section", async function() {
        await selectDimension(page, '.segmentRow1', 'Actions', 'Clicked URL');
    ...
    it("should correctly should show a confirmation when changing segment definition", async function() {
        ...
        await selectFieldValue(page, '.segmentRow1 .segment-row .metricMatchBlock', 'Is not');
